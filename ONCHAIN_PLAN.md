...
- [x] **4.4** Hook `Guest::SpendMoney` → `GUEST_SPEND` with `venueId`. Single hook in the existing `Guest::SpendMoney(money64& peep_expend_type, money64 amount, ExpenditureType, uint32_t venueId)` impl (`Guest.cpp:2349`) — fires after the existing `FinancePayment` / `MoneyEffect` / sound side-effects so Outbox emit doesn't change game-side semantics. Same triple-gate as M4.3 (`#ifdef OPENRCT2_CHAIN` + `gOpenRCT2ChainEnabled` + non-null `Chain::GetOutbox()`). New required `venueId` parameter on both `SpendMoney` overloads — caller must supply because the venue context is only known at the call-site, not derivable from `ExpenditureType` alone (food vs souvenir vs ride-fare vs facility all share `parkRideTickets` once they reach the spend impl). All four call-sites updated: (1) **`Guest.cpp:1752`** shop purchase path (food/drink/souvenir, fed by `OnEnterShop`-style logic) → `ride.id.ToUnderlying() + 1`; (2) **`Guest.cpp:3917`** `UpdateRideFreeVehicleEnterRide` ride-fare → `ride.id.ToUnderlying() + 1`; (3) **`Peep.cpp:1891`** park-entrance fee → `Chain::kVenueIdEntrance` (= 0); (4) **`Peep.cpp:2263`** facility entry → `ride->id.ToUnderlying() + 1`. New constants in `chain/Outbox.h` (alongside `VenueKind`/`SpendCategory`): `kVenueIdEntrance = 0` reserves the park-entrance slot per plan §3.2; `kVenueIdNone = UINT32_MAX` is a producer-side "no venue, skip emit" sentinel never put on the wire — useful for any future call-site that needs to call `SpendMoney` without driving the on-chain pipeline. The `+1` offset on every ride-derived id avoids colliding with the entrance sentinel since `RideId` underlying is uint16. `ExpenditureType` → `SpendCategory` mapping happens inside the hook: `parkEntranceTickets`→`Entry`, `foodDrinkSales`/`foodDrinkStock`→`ShopPrimary`, `shopSales`/`shopStock`→`ShopSecondary`, `parkRideTickets`+default→`RideFare`. We deliberately fold the rare `FacilityUse` case into `RideFare` for v0 (`ExpenditureType` doesn't distinguish facilities from rides; both ride into `parkRideTickets`); the throughput demo doesn't depend on the distinction, and the on-chain category is informational. `gameTick` comes from `getGameState().currentTicks`. `Peep.cpp` grew a `#include "../chain/Outbox.h"` (gated) so `kVenueIdEntrance` is in scope at the entrance call-site. Verification: full `libopenrct2` rebuild green with `OPENRCT2_CHAIN=ON` (signature change ripples through every TU including `Guest.h` — relinks clean); M4.1 roundtrip harness re-run, 8/8 lines parse.
- [x] **4.5** Hook `LeavingPark` → `GUEST_EXIT`. Single hook in `PeepEntityRemove(Peep*)` (`Peep.cpp:694`) — the chokepoint every guest-removal path funnels through. Chosen over hooking `Guest::UpdateLeavingPark` directly because the plan's `GUEST_EXIT` is really "guest is gone" rather than "decided to leave"; the sidecar sweeper (M3.7) needs to fire whether the guest exited the gate (`Guest.cpp:5939` → `PeepEntityRemove`), turned around before entering (`Guest.cpp:5909`), died in a ride accident (`Vehicle.cpp:4579` → `PeepEntityRemove` directly), or was removed by the cheat / console `guest remove` (`Peep::Remove` → `PeepEntityRemove`). Hook fires *before* `entities.EntityRemove(peep)` so `peep->Id` and `guest->HdIndex` are still readable; gated on `peep->As<Guest>() != nullptr` so staff removals (`StaffFireAction.cpp:80` → `PeepEntityRemove`) don't slip through. Triple-gate as in M4.3/M4.4. **Chain-enrollment edge case:** v0 deliberately doesn't track per-guest chain-enrollment (would need a flag bit; we don't have one), so a phantom-emit fires for any guest who didn't have chain identity (e.g., guests loaded from an option-(b) save while chain mode is on). The sweeper handles this gracefully — it reads `parkToken.balanceOf(guestAddress)` first and skips zero-balance addresses via the `zeroBalanceExits` counter (M3.7), so a stray exit for an unenrolled `hdIndex` just bumps a metric. The save-loading-into-chain-mode case is documented as out-of-scope for v0 anyway. Verification: full `libopenrct2` rebuild green; M4.1 roundtrip 8/8 still parsing.
- [x] **4.6** Hook ride/shop/facility placement → `VENUE_REGISTERED`. Single hook at the end of `RideCreateAction::Execute(GameState_t&)` in `src/openrct2/actions/RideCreateAction.cpp` — the canonical chokepoint for *every* ride/shop/stall/facility/ATM placement. Same triple-gate as M4.3–M4.5. Hook fires after `res.SetData(RideId{rideIndex})` so the ride is fully initialised (name defaulted, colours set, prices populated) before the WAL line goes out. **VenueKind classification:** mapped from the RideTypeDescriptor's `RideColourKey` field (chosen because it's already the OpenRCT2-internal taxonomy of "what kind of revenue-producing thing is this" — populated by every shop/stall/facility's RTD definition, e.g. `ride/rtd/shops/CashMachine.h`, `InformationKiosk.h`, etc.). Mapping: `Food` / `Drink` → `Stall`, `Shop` → `Shop`, `InfoKiosk` / `FirstAid` / `Toilets` → `Facility`, `CashMachine` → `ATM`, `Ride` (and default) → `Ride`. **Object identifier:** `objectType` field is the OpenRCT2 object identifier (e.g. `"rct2.ride.bms"`) pulled via `GetContext()->GetObjectManager().GetLoadedObject<RideObject>(rideEntryIndex)->GetIdentifier()`. Falls back to empty string if the object lookup fails (defensive — should never happen since `Execute` already validated `rideEntry`). **Name:** `ride->getName()` returns the formatted display name (e.g. `"Wooden Roller Coaster 1"`); the Outbox's M4.1 string-truncation cap (64 B) handles any pathologically long name without breaking the WAL line. **VenueId:** `rideIndex.ToUnderlying() + 1` per the M4.4 convention (the +1 leaves 0 reserved for `kVenueIdEntrance`). The action's `Query` path is *not* hooked — only `Execute` results in real on-chain register calls (per `VenueRegistry`'s "register/rename/remove" lifecycle). New includes (chain-gated): `OpenRCT2.h` for `gOpenRCT2ChainEnabled`, `chain/Outbox.h`, `object/RideObject.h`. Verification: full `libopenrct2` rebuild green; M4.1 roundtrip 8/8 still parsing; the Outbox.cpp serializer's `VENUE_REGISTERED` JSON shape (`venueId` + `venueKind` + `name` + `objectType`) is the same one M4.1 already round-tripped end-to-end against the sidecar's `parseEvent`.
- [x] **4.7** Hook ride/shop rename → `VENUE_RENAMED`. Hook in `RideSetNameAction::Execute(GameState_t&)` (`src/openrct2/actions/RideSetNameAction.cpp:97` area), gated identically to M4.6. Fires after the name commit (`ride->customName = _name;` or `ride->setNameToDefault();` at lines 77/81) so `ride->getName()` returns the *new* name. `venueId = _rideIndex.ToUnderlying() + 1` per the M4.4/M4.6 convention. New chain-gated includes: `OpenRCT2.h` for `gOpenRCT2ChainEnabled`, `chain/Outbox.h`. Verification: full `libopenrct2` rebuild green; M4.1 roundtrip 8/8 still parsing (covers `VENUE_RENAMED` shape).
- [x] **4.8** Hook ride/shop demolish → `VENUE_REMOVED`. Hook in `RideDemolishAction::DemolishRide(GameState_t&, Ride&)` just before `return res;` (around `RideDemolishAction.cpp:177`). Deliberately scoped to the demolish branch — `Execute` dispatches on `_modifyType` to either `DemolishRide` or `RefurbishRide`; refurbish doesn't remove the venue, so we don't fire there. The `rideId` capture earlier in the function (`const auto rideId = ride.id;` at line 135) means we can fire post-`ride.remove()` (line 158) without a use-after-free; we go even later (after `ScrollingTextInvalidate()`) so the WAL emit is the very last thing before the result returns. `venueId = rideId.ToUnderlying() + 1` per convention. Same chain-gated includes as M4.7. Verification: full `libopenrct2` rebuild green; M4.1 roundtrip 8/8 still parsing (covers `VENUE_REMOVED` shape).
- [ ] **4.9** Park-side hooks for wages/marketing/construction → batched park flow.
- [x] **4.10** Auto-spawn sidecar from `AIAgentLaunch.cpp`. The integration step that finally makes M4.1–M4.8 *active* — until 4.10 lands, every game-side hook is correct-but-inert (`Chain::GetOutbox()` returns nullptr because nothing has called `SetOutbox(...)`). Two new files split the responsibility: **`src/openrct2/chain/Sidecar.{h,cpp}`** is a minimal POSIX subprocess wrapper (`Linux`/`macOS`/BSD only — matches `terminal/AIAgentLaunch.cpp`'s existing platform gates). `Start(opts, errorOut)` is a textbook `fork+exec`: the child does `setsid()` (new session + process group so the sidecar's signals are isolated from the agent terminal's PTY — the agent CLI sends SIGINT/SIGWINCH all over its own session), redirects stdout+stderr to the configured log file (or `/dev/null`), redirects stdin from `/dev/null`, optionally `chdir`s, then `execve`s. Pre-fork it builds argv + composed env (parent env + caller's overrides) into vectors-of-strings whose `.data()` we hand to execve — no malloc-using C++ between fork and exec. `Stop()` SIGTERMs the whole process group + the leader, polls `waitpid(WNOHANG)` every 50 ms for up to 2 s of grace, then SIGKILLs. Destructor calls `Stop()` so a fall-through on game exit cleans up. **`src/openrct2/chain/Runtime.{h,cpp}`** is the orchestrator. `EnsureRuntime(repoRoot, workspace)` is idempotent — first call constructs the global `Outbox`, calls `Outbox::Start()`, threads it through `Chain::SetOutbox()`, then `fork+exec`s the sidecar with the matching paths; subsequent calls no-op while the runtime is up. Paths land under `<workspace>/chain/`: `outbox.wal`, `outbox.cursor`, `sidecar.sock`, `sidecar.log`, `keystore.json` (the keystore default; caller can override via `RuntimeOptions::keystorePath`). Sidecar argv: `node <repoRoot>/chain-sidecar/dist/main.js --socket <sock> --outbox <wal> --outbox-cursor <cursor> --keystore <keystore>`, plus optional `--rpc-url` / `--deployments` / `--faucet-owner-keyfile` lifted from env (`MONAD_RPC_URL` / `MONAD_DEPLOYMENTS` / `FAUCET_OWNER_KEYFILE`) so the user can override without touching C++. Credentials (`KEYSTORE_PASSPHRASE`, `FAUCET_OWNER_KEY`) flow through inherited environment — the sidecar's M2.2/M2.5 CLI surface reads them itself; v0 doesn't prompt or stash on the C++ side. `TeardownRuntime()` SIGTERMs sidecar then drops the Outbox singleton (which closes the WAL via destructor); v0 doesn't actually call this — sidecar lives until process exit, relying on kernel-side reaping. Restart-on-crash supervision is intentionally not built; if the sidecar dies the WAL keeps growing on disk and the next sidecar boot replays from the cursor. **Wiring** is a single chain-gated block in `BuildAIAgentLaunchPlan` (`AIAgentLaunch.cpp`) — placed right after the `repoRoot` + `workspace` resolution so the runtime gets the same paths the agent CLI is already using; failure logs a warning but doesn't block the agent launch (chain mode stays opt-in). Looking up the sidecar entry at `<repoRoot>/chain-sidecar/dist/main.js` matches the build contract — `cmake --build build --target agent_bundle` already runs `npm run build` to produce that file, so a dev who has the agent terminal working has a buildable sidecar. **What 4.10 deliberately doesn't do:** (a) no inbound IPC client on the C++ side — plan §5.3's pushed `chain.event.push` events have no v0 consumer (in-game UI is M6, deferred); (b) no per-window restart on terminal close — sidecar lifecycle is "until process exit" since reopening the terminal would clobber the global Outbox via a re-`SetOutbox`; (c) no Windows path — chain mode is Linux/macOS-only per CLAUDE.md, and the AIAgentLaunch flow is similarly platform-gated. **Verification:** full `libopenrct2` rebuild green with `OPENRCT2_CHAIN=ON` (forks the existing single-TU recompile pattern; the new chain/Runtime + chain/Sidecar TUs get pulled into the libopenrct2 glob and link clean); M4.1 roundtrip 8/8 still parsing. Live testnet exercise (open the agent terminal with `--chain`, see `sidecar.log`, see `outbox.wal` accumulate as guests spawn / spend, see relayer pool submit settle txs) is the M4-acceptance test — staged for the user to drive against testnet now that the wiring is end-to-end.
